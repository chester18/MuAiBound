...
public class DetailInsurancePolicyActivity extends CBaseActivity {
    ...
    InsurancePolicyModel model;            //数据模型
    String updataUrl;               //上传地址
    String[] mainTitle={"保险公司:","保险单号:","险种:","有效期:","照片:",""};
    String[] keys={"PolicyName","PolicyNo","PolicyType","ExpiryDate","PolicyImg"};
    BaseAdapter adapter=new BaseAdapter() {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            if (i==keys.length)
            {
                ButtonCell bcell=new ButtonCell(DetailInsurancePolicyActivity.this);
            ...
            IcoCell cell=new IcoCell(DetailInsurancePolicyActivity.this);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        model= (InsurancePolicyModel) getIntent().getSerializableExtra("model");
        if (model==null)
        {
            model=new InsurancePolicyModel(new JsonObject());
        ...
        switch (position){
            case 0:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_INPUT, 0, mainTitle[position], "", new DialogCallBackInterface() {
                    ...
                    public void selectShure(int tag, Object result) {
                        model.PolicyName=result.toString();
            ...
            case 1:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_INPUT, 1, mainTitle[position], "", new DialogCallBackInterface() {
                    @Override
                    public void selectShure(int tag, Object result) {
                        model.PolicyNo=result.toString();
                        adapter.notifyDataSetChanged();
                    }
            ...
            case 2:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_INPUT, 2, mainTitle[position], "", new DialogCallBackInterface() {
                    @Override
                    public void selectShure(int tag, Object result) {
                        model.PolicyType=result.toString();
                        adapter.notifyDataSetChanged();
                    }
            ...
            case 3:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_DATE, 3, mainTitle[position], "", new DialogCallBackInterface() {
                    @Override
                    public void selectShure(int tag, Object result) {
                        model.ExpiryDate=result.toString();
                        adapter.notifyDataSetChanged();
                    }
            ...
            default:
                sendPost(updataUrl,model.getJsonObject(),"信息提交中...",1);
                break;
    ...
    protected void getResult(String result, int tag) {
        ...
        if (tag==4)
        {
            ...
            if (jo.get("result").getAsInt()==1)
            {
                ...
                model.PolicyImg=img.get("name").getAsString();
                adapter.notifyDataSetChanged();
