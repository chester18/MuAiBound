...
public class DetailMedicalExaminationActivity extends CBaseActivity {
    ...
    MedicalExaminationModel model;            //数据模型
    String updataUrl;               //上传地址
    String[] mainTitle={"医疗机构:","检查科目:","检查结果:","体检时间:","结果照片:",""};
    String[] keys={"MedicalOrga","CheckObject","CheckResual","CheckDate","checkImg"};
    BaseAdapter adapter=new BaseAdapter() {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            {
                ButtonCell bcell=new ButtonCell(DetailMedicalExaminationActivity.this);
            ...
            IcoCell cell=new IcoCell(DetailMedicalExaminationActivity.this);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        model= (MedicalExaminationModel) getIntent().getSerializableExtra("model");
        if (model==null)
        {
            model=new MedicalExaminationModel(new JsonObject());
            model.userId= PubFunction.userid;
            updataUrl= CommonNetString.userAddMedical;
        }else {
            updataUrl=CommonNetString.userUpdateMedical;
        }
        title.setText("体检记录");
        ...
        switch (position){
            case 0:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_INPUT, 0, mainTitle[position], "", new DialogCallBackInterface() {
                    ...
                    public void selectShure(int tag, Object result) {
                        model.MedicalOrga=result.toString();
            ...
            case 1:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_INPUT, 1, mainTitle[position], "", new DialogCallBackInterface() {
                    @Override
                    public void selectShure(int tag, Object result) {
                        model.CheckObject=result.toString();
                        adapter.notifyDataSetChanged();
                    }
            ...
            case 2:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_INPUT, 2, mainTitle[position], "", new DialogCallBackInterface() {
                    @Override
                    public void selectShure(int tag, Object result) {
                        model.CheckResual=result.toString();
                        adapter.notifyDataSetChanged();
                    }
            ...
            case 3:
            {
                BaseDialog bd= DialogFactry.instance(this, DialogFactry.DIA_DATE, 3, mainTitle[position], "", new DialogCallBackInterface() {
                    @Override
                    public void selectShure(int tag, Object result) {
                        model.CheckDate=result.toString();
                        adapter.notifyDataSetChanged();
                    }
    ...
    protected void getResult(String result, int tag) {
        ...
        if (tag==4)
        {
            ...
            if (jo.get("result").getAsInt()==1)
            {
                ...
                model.checkImg=img.get("name").getAsString();
                adapter.notifyDataSetChanged();
